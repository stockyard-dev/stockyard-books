...
func (d *DB) GetAccount(id string) *Account {
	var a Account
	if err := d.db.QueryRow(`SELECT id,name,type,currency,created_at FROM accounts WHERE id=?`, id).Scan(&a.ID, &a.Name, &a.Type, &a.Currency, &a.CreatedAt); err != nil {
		return nil
	}
...
func (d *DB) UpdateAccount(id string, a *Account) error {
	_, err := d.db.Exec(`UPDATE accounts SET name=?,type=?,currency=? WHERE id=?`, a.Name, a.Type, a.Currency, id)
	return err
}
