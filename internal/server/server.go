...
func New(db *store.DB, limits Limits, dataDir string) *Server {
	...
	s.mux.HandleFunc("GET /api/tier", func(w http.ResponseWriter, r *http.Request) {
		writeJSON(w, 200, map[string]any{"tier": s.limits.Tier, "upgrade_url": "https://stockyard.dev/books/"})
	})
...
func (s *Server) listAccounts(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, 200, map[string]any{"accounts": orEmpty(s.db.ListAccounts())})
}
func (s *Server) createAccount(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, 201, s.db.GetAccount(a.ID))
}
func (s *Server) getAccount(w http.ResponseWriter, r *http.Request) {
	a := s.db.GetAccount(r.PathValue("id"))
	if a == nil {
...
func (s *Server) updateAccount(w http.ResponseWriter, r *http.Request) {
	...
	if a.Currency == "" {
		a.Currency = ex.Currency
	}
	s.db.UpdateAccount(id, &a)
	writeJSON(w, 200, s.db.GetAccount(id))
}
...
func (s *Server) listTransactions(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, 200, map[string]any{"transactions": orEmpty(s.db.ListTransactions(100))})
}
func (s *Server) createTransaction(w http.ResponseWriter, r *http.Request) {
	var t store.Transaction
	json.NewDecoder(r.Body).Decode(&t)
	if t.Amount <= 0 {
		writeErr(w, 400, "amount required")
		return
	}
	if t.DebitAcct == "" || t.CreditAcct == "" {
		writeErr(w, 400, "debit and credit accounts required")
		return
	}
	s.db.CreateTransaction(&t)
	writeJSON(w, 201, t)
}
...
func (s *Server) profitLoss(w http.ResponseWriter, r *http.Request) {
	writeJSON(w, 200, s.db.ProfitLoss())
}
func (s *Server) stats(w http.ResponseWriter, r *http.Request) { writeJSON(w, 200, s.db.Stats()) }
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	st := s.db.Stats()
	writeJSON(w, 200, map[string]any{"status": "ok", "service": "books", "accounts": st.Accounts, "transactions": st.Transactions})
}
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("%s: warning: could not parse config.json: %v", "books", err)
		return
	}
	s.pCfg = cfg
	log.Printf("%s: loaded personalization from %s", "books", path)
}
